...
import com.bkotharu.examples.UserRegistryMessages.ActionPerformed;
import com.bkotharu.examples.UserRegistryMessages.CreateUser;
...
public class UserRoutes extends AllDirectives {
	...
	public Route routes() {
		return pathPrefix("users", () -> route(getOrPostUsers(),
				path(PathMatchers.segment(), name -> route(getUser(name), deleteUser(name)))));
	}
	...
	private Route getUser(String name) {
		...
		Route getRoute = get(() -> {
			CompletionStage<Optional<User>> user = PatternsCS
					.ask(userActorRef, new UserRegistryMessages.GetUser(name), timeout)
	...
	private Route deleteUser(String name) {
		...
		Route deleteRoute = delete(() -> {
			CompletionStage<ActionPerformed> userDeleted = PatternsCS
					.ask(userActorRef, new UserRegistryMessages.DeleteUser(name), timeout)
	...
	private Route getOrPostUsers() {
		return pathEnd(() -> route(get(() -> {
			CompletionStage<Users> futureUsers = PatternsCS
					.ask(userActorRef, new UserRegistryMessages.GetUsers(), timeout).thenApply(obj -> (Users) obj);
